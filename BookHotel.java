...
public class BookHotel extends JFrame implements ActionListener {
    ...
    BookHotel(String username) {
        ...
        // Disable the "Book Hotel" button initially
        bookpackage.setEnabled(false);
    ...
    if (ae.getSource() == checkprice) {
        try {
            // Parse the values for number of persons and days
            int persons = 0;
            int days = 0;
            try {
                ...
                JOptionPane.showMessageDialog(null, "Please Enter Valid Integer Values for Persons and Days");
                return; // Exit the method if parsing fails
            }

            // Validate if persons and days are greater than 0
            if (persons <= 0 || days <= 0) {
                JOptionPane.showMessageDialog(null, "Please Enter Valid Values for Persons and Days (greater than 0)");
                return; // Exit the method if values are not valid
            }

            // Proceed with price calculation if validation passes
            Conn c = new Conn();
